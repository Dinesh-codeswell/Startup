...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`Saving ${teams.length} teams and ${participants.length} participants to database...`);
    ...
    for (const participant of allParticipants) {
      try {
        ...
        if (existingSubmission) {
          ...
          console.log(`Found existing submission for ${participant.email}: ${submissionId}`);
        } else {
          ...
          const submissionData = {
            ...
            whatsapp_number: participant.whatsappNumber || participant.whatsapp || '',
            college_name: participant.collegeName || participant.college || '',
            current_year: participant.currentYear || participant.year || 'Unknown',
          ...
          console.log(`Creating new submission for ${participant.email}:`, submissionData);
          ...
          if (submissionError) {
            ...
            errors.push(`Failed to create submission for ${participant.email}: ${submissionError.message}`);
            continue;
          }

          submissionId = newSubmission.id;
          console.log(`✅ Created new submission: ${submissionId} for ${participant.email}`);
        }
        ...
        console.error(`Error processing participant ${participant.fullName || participant.email}:`, error);
        errors.push(`Error processing ${participant.email}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
      ...
      try {
        ...
        const memberSubmissionIds = team.members
          .map((member: any) => {
            ...
            if (!saved) {
              console.warn(`No submission found for team member ${member.email} (${member.fullName})`);
              console.warn(`Available emails in savedSubmissions:`, savedSubmissions.map(s => s.participantEmail));
            } else {
        ...
        console.log(`Team ${team.id}: Found ${memberSubmissionIds.length} valid submission IDs out of ${team.members?.length || 0} members`);
        ...
        errors.push(`Error saving team ${team.id}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
    ...
    console.log(`Found ${matchedSubmissionIds.length} matched submission IDs to update:`, matchedSubmissionIds);

    if (matchedSubmissionIds.length > 0) {
      console.log(`Updating ${matchedSubmissionIds.length} submissions to 'team_formed' status`);
      ...
      if (checkError) {
        ...
        console.log(`Found ${existingSubmissions?.length || 0} existing submissions to update`);
      ...
      if (updateError) {
        ...
        errors.push(`Error updating submission statuses: ${updateError.message}`);
      } else {
        console.log(`✅ Successfully updated ${updatedSubmissions?.length || 0} submissions to team_formed status`);
        updatedSubmissions?.forEach(sub => {
          console.log(`  - ${sub.email}: new status = ${sub.status}`);
        });
      }
    } else {
      console.warn('⚠️ No matched submission IDs found - status update skipped');
    ...
    try {
      ...
      for (const team of savedTeams) {
        try {
          ...
          console.warn(`Failed to send notification for team ${team.team_name}:`, notifError);
        }
      }
