...
export default function SignupPage() {
 ...
 // Add this useEffect to handle URL redirect parameter
useEffect(() => {
  const urlParams = new URLSearchParams(window.location.search);
  const redirectParam = urlParams.get('redirect');
  
  if (redirectParam) {
    sessionStorage.setItem('redirectAfterSignup', decodeURIComponent(redirectParam));
  }
}, []);
 ...
 const handleSubmit = async (e: React.FormEvent) => {
  ...
  // Check if user was created successfully AND is signed in
    if (result.error && result.error.message.includes("User already registered")) {
      setError("An account with this email already exists. Please sign in instead.")
    } else if (result.user && result.session) {
      // User is successfully signed up AND signed in
      ...
      if (redirectUrl) {
        // Wait a moment for success message, then redirect to resource
        setTimeout(() => {
          sessionStorage.removeItem("redirectAfterSignup")
          router.push(redirectUrl) // Changed from window.open
        }, 1500)
      } else {
      ...
      // User was created but not signed in (shouldn't happen with our updated auth)
 ...
 // Update the handleSocialSignup function:
const handleSocialSignup = async (provider: string) => {
  setIsLoading(true)
  setError("")

    try {
    // Get redirect URL to pass through OAuth flow
    const redirectUrl = sessionStorage.getItem("redirectAfterSignup")
